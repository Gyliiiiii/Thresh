...
namespace Thresh.Core.Config.INI
{
    ...
    public sealed class Section : ConfigurationElement, IEnumerable<Setting>
    {
        ...
        public void Remove(string settingName)
        {
            ...
            if (setting == null)
                throw new ArgumentException(string.Format("A setting named '{0}' does not exist in the section.",
                    settingName));
        ...
        public string ToString(bool includeComment)
        {
            if (includeComment)
            {
                ...
                string[] preCommentStrings =
                    hasPreComments ? mPreComments.ConvertAll(c => c.ToString()).ToArray() : null;
