...
namespace Thresh.Unity.Utility
{
    ...
    {
        public static int TerrianLayerMask = 1 << LayerMask.NameToLayer("Terrain");

        public static int SceneObjectLayerMask = TerrianLayerMask | 1 << LayerMask.NameToLayer("Default");
        ...
        public static bool RayCastTerrain(Ray r, int layerMask, out Vector3 pos)
        {
            ...
            int lmask = layerMask == -1 ? TerrianLayerMask : layerMask;
        ...
        public static void ShowGameObject(this GameObject obj)
        {
            obj.transform.localScale = Vector3.one;
        }

        public static void ShowGameObject(this GameObject obj, bool show)
        {
            if (show) obj.ShowGameObject();
            else obj.HideGameObject();
        }

        public static void HideGameObject(this GameObject obj)
        {
        ...
        public static void SetImage(GameObject obj, string name, string path)
        {
            GetComponent<Image>(obj, name).sprite = GetSprite(path);
        }

        public static void SetParent(this Transform child, Transform parent, Vector3 position, Quaternion rotation,
            Vector3 scale)
        {
        ...
        public static void SetRectParent(this Transform child, GameObject parent)
        {
        ...
        {
            Sprite sp;
            if (ImageCache.TryGetValue(path,out sp))
            {
                return sp;
            }
            ...
            string tpath = path.Substring(0, index);
            string name = path.Substring(index + 1, path.Length - index - 1);
            Sprite[] sps = Resources.LoadAll<Sprite>(tpath);
            for (int i = 0; i < sps.Length; i++)
            {
                ImageCache[sps[i].name] = sps[i];
                if (sps[i].name == name)
                {
                    sp = sps[i];
                }
